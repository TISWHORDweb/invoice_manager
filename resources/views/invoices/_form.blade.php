<form method="post" action="{{ $action }}" id="invoice-form">
    ...
    <div class="form-group">
        ...
        <textarea id="client_address" name="client_address" rows="2">{{ old('client_address', $invoice->client_address) }}</textarea>
    </div>
    ...
    <div class="card-header" style="margin:1.5rem 0 1rem 0;padding:0;border:0;">Line items</div>
            ...
            <tbody>
                ...
                @foreach($items as $idx => $item)
                    <tr>
                        ...
                        <td class="item-amount text-right num">0.00</td>
                        <td>
                            @if(!empty($item['id']))
    ...
    <button type="button" id="add-row" class="btn btn-outline" style="margin-top:0.75rem;">Add line</button>
...
(function() {
    ...
    addBtn.addEventListener('click', function() {
        ...
        tr.innerHTML = '<td><input type="text" name="items[' + rowIndex + '][description]" required></td>' +
            ...
            '<td class="item-amount text-right num">0.00</td>' +
            '<td><button type="button" class="btn btn-outline remove-row">Remove</button></td>';
